...
class ModelHandler(object):
    ...
    def __init__(self, config):
        ...
        # only test
        self.logger = DummyLogger(config, dirname=config['out_dir'], pretrained=config['pretrained'])
        self.dirname = config['model_dir']
        # self.dirname = '../out/squad-split2/ga-seq2seq_split2'
        # self.dirname = '../out/squad-split1/ga-seq2seq_split1'
        # self.dirname = '../out/marco1/ga-seq2seq_marco1'
        ...
        if config['use_bert']:
            ...
            bert_tokenizer = BertTokenizer.from_pretrained(config['bert_model'], do_lower_case=True) #BertTokenizer对输入文本进行处理
            ...
            if not config.get('finetune_bert', None):
                ...
                print('[ Finetune BERT layers ]') #微调bert layers
        else:
            bert_tokenizer = None
    ...
    def train(self):
        ...
        while self._stop_condition(self._epoch, self.config['patience']):
            ...
            format_str += self.metric_to_str(self._train_metrics) #mean
            ...
            if self._best_metrics[self.config['eary_stop_metric']] <= self._dev_metrics[self.config['eary_stop_metric']].mean():
                ...
                for k in self._dev_metrics:
                    self._best_metrics[k] = self._dev_metrics[k].mean() #mean
    ...
    def test(self):
        ...
        if self.bert_model is not None: #测试时bert参数固定
            for param in self.bert_model.parameters():
                param.requires_grad = False
        ...
        metrics = evaluate_predictions(gold, output) #mean
    ...
    def _update_metrics(self, loss, metrics, batch_size, training=True):
        if training:
            if loss:
                self._train_loss.update(loss) #update()更新字
