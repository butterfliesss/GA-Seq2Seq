...
from .models.gaseq2seq import GASeq2Seq
...
class Model(object):
    ...
    def __init__(self, config, train_set=None):
        self.config = config
        if config['model_name'] == 'ga-seq2seq':
            self.net_module = GASeq2Seq
        else:
    ...
    def predict(self, batch, step, forcing_ratio=1, rl_ratio=0, update=True, out_predictions=False, mode='train'):
        ...
        if mode == 'train':
            ...
            decoded_batch, loss_value, metrics = dev_batch(batch, self.network, self.vocab_model.word_vocab, self.criterion, show_cover_loss=self.config['show_cover_loss'])
...
def beam_search(batch, network, vocab, config):
    ...
    to_decode = [[101141] if each==[] else each[0].tokens[1:] for each in hypotheses] # the first token is SOS
...
def batch_beam_search(network, ex, ext_vocab_size=None, beam_size=4, *,
    ...
    a_enc = network.input_encoder(a, input_lengths)[0].transpose(0, 1)
    b_enc = network.input_encoder(b, answer_lengths)[0].transpose(0, 1)
    align_a, align_b = network.alignment(a_enc, b_enc, mask_a, mask_b)
    a = network.fusion(a_enc, align_a)
    a_final = network.ctx_encoder(a, input_lengths)[0].transpose(0, 1)

    encoder_outputs = a_final

    if network.name == 'GASeq2Seq':
      ...
      myseq_encoder_outputs, myseq_encoder_state = network.aft_graph_rnn(node_embedding.transpose(0, 1), input_lengths)
