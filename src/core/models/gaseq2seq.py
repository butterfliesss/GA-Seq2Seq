# Copied from https://github.com/hugochan/RL-based-Graph2Seq-for-NQG
__author__ = "Yu Chen"
...
# Changelog:
# Propose a graph augmented passage encode containing a graph encoder and a sequence encoder.
# Design an answer-aware passage representation module.
...
from ..layers.common import EncoderRNN, DecoderRNN, dropout, GatedFusion
...
class GASeq2Seq(nn.Module):
  ...
  def __init__(self, config, word_embedding, word_vocab):
    ...
    self.input_encoder = EncoderRNN(enc_input_dim , enc_hidden_size, bidirectional=config['enc_bidi'], #enc_hidden_size
                                    num_layers=config['num_enc_rnn_layers'], rnn_type=self.rnn_type,
                            rnn_dropout=config['enc_rnn_dropout'], device=self.device)
    self.alignment = alignment['identity'](1, enc_hidden_size, 0.2)
    self.fusion = fusion['my'](enc_hidden_size, enc_hidden_size)
    self.ctx_encoder = EncoderRNN(enc_hidden_size, enc_hidden_size, bidirectional=config['enc_bidi'],
                                    num_layers=config['num_enc_rnn_layers'], rnn_type=self.rnn_type,
                            rnn_dropout=config['enc_rnn_dropout'], device=self.device) #config['enc_rnn_dropout']
    

    self.aft_graph_rnn = EncoderRNN(enc_hidden_size, enc_hidden_size, bidirectional=config['enc_bidi'],
                                    num_layers=config['num_enc_rnn_layers'], rnn_type=self.rnn_type,
                                    rnn_dropout=config['enc_rnn_dropout'], device=self.device)
  ...
  def forward(self, ex, target_tensor=None, criterion=None, criterion_reduction=True, criterion_nll_only=False, \
              ...
              ext_vocab_size=None, sample=False, saved_out: GASeq2SeqOutput=None, \
              visualize: bool=None, include_cover_loss: bool=False) -> GASeq2SeqOutput:
    """
    ...
    """
    input_tensor = ex['context'] #[batch, seq_len]
    input_lengths = ex['context_lens']

    batch_size, input_length = input_tensor.shape
    input_mask = create_mask(input_lengths, input_length, self.device) #[batch, seq_len]
    ...
    if target_tensor is None:
      ...
      target_mask = create_mask(ex['target_lens'], target_length, self.device)  #passage mask
    ...
    if saved_out:  # reuse encoder states of a previous run
      ...
      enc_input_cat = [encoder_embedded]   #passage inputs
      if self.f_case:
      ...
      if self.f_ans:
        ...
        enc_answer_cat = [ans_embedded]  #answer inputs
        ...
        if self.use_bert:
          ...
          answer_bert = torch.mm(weights_bert_layers, answer_bert.view(answer_bert.size(0), -1)).view(
            answer_bert.shape[1:])
      ...
      a = torch.cat(enc_input_cat, -1)
      b = torch.cat(enc_answer_cat, -1)
      mask_a = input_mask.unsqueeze(2)
      mask_b = ans_mask.unsqueeze(2)

      a_enc = self.input_encoder(a, input_lengths)[0].transpose(0, 1)
      b_enc = self.input_encoder(b, answer_lengths)[0].transpose(0, 1)
      align_a, align_b = self.alignment(a_enc, b_enc, mask_a, mask_b)
      a = self.fusion(a_enc, align_a)
      a = self.ctx_encoder(a, input_lengths)[0].transpose(0, 1)
      a_final = a

      encoder_outputs = a_final
      ...
      myseq_encoder_outputs, myseq_encoder_state = self.aft_graph_rnn(node_embedding.transpose(0, 1), input_lengths)
      #[max_seqlen, batch, dim], [1, batch, dim]
      init_state = myseq_encoder_state
      encoder_outputs = myseq_encoder_outputs
      encoder_state = init_state


    # initialize return values
    r = GASeq2SeqOutput(encoder_outputs, encoder_state,
